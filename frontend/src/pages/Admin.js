...
<<<<<<< HEAD
      ...
      if (resPapeis.status === 'fulfilled') setPapeis(resPapeis.value.data);
      else console.error('Erro ao carregar papéis:', resPapeis.reason);

      if (resHospitais.status === 'fulfilled') setHospitais(resHospitais.value.data);
      else console.error('Erro ao carregar hospitais:', resHospitais.reason);

      if (resUsers.status === 'fulfilled') {
        console.log('Utilizadores carregados:', resUsers.value.data.length);
        setUtilizadores(resUsers.value.data);
      } else {
        console.error('Erro ao carregar utilizadores:', resUsers.reason);
      }

      if (resUtentes.status === 'fulfilled') setUtentes(resUtentes.value.data);
      if (resEpisodios.status === 'fulfilled') setEpisodios(resEpisodios.value.data);
